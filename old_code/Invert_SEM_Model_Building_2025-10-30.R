...
# removed vegetation cover to avoid model convergence issues
...
beta_std # -0.239 is standardized estimate (with buffer presence it's -0.318)
...
sd_x <- sd(invert$PercentAg)
beta_std <- beta * (sd_x / sd_y)
beta_std #-0.103  is standardized estimate (with buffer presence it's -0.124)
...
sd_x <- sd(invert$PercentAg)
beta_std <- beta * (sd_x / sd_y)
beta_std # 0.0396 is standardized estimate (with buffer presence it's 0.0412)
...
sd_x <- sd(invert$PercentAg)
beta_std <- beta * (sd_x / sd_y)
beta_std # -0.0316 is standardized estimate (with buffer presence it's -0.037)
...
sd_x <- sd(invert$PercentAg)
beta_std <- beta * (sd_x / sd_y)
beta_std # -0.0146 is standardized estimate (with buffer presence it's -0.0134)
...
sd_x <- sd(invert$PercentAg)
beta_std <- beta * (sd_x / sd_y)
beta_std # 0.045 is standardized estimate (with buffer presence it's 0.047)
...
# data is not dispersed enough to need negative binomial -- get warnings
# m2 <- glm.nb(Diversity ~ PercentAg + Season + LogConductivity + 
#             EnvDetection + Permanence + pH_probe + Buffered, 
#           data = invert)
...
# ggplot(invert, aes(x = as.factor(EnvDetection), 
#                    y = PrecipitationAmount_7days)) +
